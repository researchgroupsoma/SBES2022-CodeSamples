...
from utils import output_write, get_samples
...
def create_output(framework, sample, question, answer, question_owner, answer_owner):
    ...
    question_owner_tags = list_to_string(get_top_five_tags(question_owner["user_id"]))
    answer_owner_tags = [] if answer_owner["user_id"] == "" else list_to_string(get_top_five_tags(answer_owner["user_id"]))
    ...
    for sample in samples:
        ...
        for question in questions["items"]:
            try:
                ...
                answer_owner = get_owner_by_user_id(answer["owner"]["user_id"])
            except KeyError:
                answer = {
                    "answer_id": "",
                    "score": "",
            ...
            question_owner = get_owner_by_user_id(question["owner"]["user_id"])
